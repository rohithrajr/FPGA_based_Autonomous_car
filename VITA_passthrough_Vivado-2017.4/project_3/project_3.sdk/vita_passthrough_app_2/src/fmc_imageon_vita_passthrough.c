...
//#if defined(XPAR_CFA_NUM_INSTANCES)
 ...
 //  a.Config.BaseAddress = pDemo->uBaseAddr_CFA;
 ...
 //  CFA cfa = {};
 //  CFA_Config *conf = CFA_LookupConfig(XPAR_CFA_0_DEVICE_ID);
 //  CFA_CfgInitialize(&cfa, conf, XPAR_CFA_0_BASEADDR);
 //  CFA_Reset(&cfa);
 //  CFA_Enable( &cfa );
 //  CFA_RegUpdateDisable( &cfa );
 // CFA_WriteReg( &cfa , CFA_BAYER_PHASE, 0 );
 //  CFA_RegUpdateEnable( &cfa );
//#endif
